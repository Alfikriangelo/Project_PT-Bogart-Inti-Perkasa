    ...
    if (location.state?.detail) {
      ...
      const fetchDetail = async () => {
        try {
          ...
          const docRef = doc(db, "tagihan-pdf", id);
  ...
  const calculateTotalPrice = () => {
    if (Array.isArray(detail.items)) {
      return detail.items.reduce((acc, item) => {
        return acc + item.jumlahBarang * item.hargaSatuanBarang;
      }, 0);
    }
    return 0;
  };
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              <div style={{ display: "flex" }}>
                ...
                >
                  {detail.noPo ? detail.noPo : "Tidak ada data"}
                </Typography>
              </div>
              <Typography
                style={{
                ...
                <div>
                  {Array.isArray(detail.items) ? (
                    detail.items.map((item, index) => (
                      ...
                      >
                        <Typography style={{ color: "white", fontSize: 18 }}>
                          Rp. {item.hargaSatuanBarang.toLocaleString()}
                        </Typography>
                      </div>
                    ))
                  ) : (
